...
var decodeContentBlockNodes = function decodeContentBlockNodes(
  blocks,
  entityMap
) {
    ...
    .reduce(function(blockMap, block, index) {
      !Array.isArray(block.children)
        ? process.env.NODE_ENV !== "production"
          ...
          : invariant(false)
        : void 0; // ensure children have valid keys to enable sibling links
      ...
      while (stack.length > 0) {
        ...
        var _contentBlockNode = new ContentBlockNode(
          _objectSpread({}, decodeBlockNodeConfig(node, entityMap), {
            ...
            nextSibling:
              _index === siblings.size - 1 ? null : siblings.get(_index + 1)
          })
...
var decodeRawBlocks = function decodeRawBlocks(rawState, entityMap) {
  ...
  if (process.env.NODE_ENV !== "production") {
    !DraftTreeInvariants.isValidTree(blockMap)
      ? process.env.NODE_ENV !== "production"
        ? invariant(false, "Should be a valid tree")
        : invariant(false)
      : void 0;
  }
...
var convertFromRawToDraftState = function convertFromRawToDraftState(rawState) {
  !Array.isArray(rawState.blocks)
    ? process.env.NODE_ENV !== "production"
      ? invariant(false, "invalid RawDraftContentState")
      : invariant(false)
    : void 0; // decode entities
