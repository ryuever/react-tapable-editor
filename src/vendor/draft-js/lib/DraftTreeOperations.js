...
var verifyTree = function verifyTree(tree) {
  if (process.env.NODE_ENV !== "production") {
    !DraftTreeInvariants.isValidTree(tree)
      ? process.env.NODE_ENV !== "production"
        ? invariant(false, "The tree is not valid")
        : invariant(false)
      : void 0;
  }
};
...
var updateParentChild = function updateParentChild(
  blockMap,
  parentKey,
  childKey,
  position
  ...
  !(parent != null && child != null)
    ? process.env.NODE_ENV !== "production"
      ? invariant(false, "parent & child should exist in the block map")
      : invariant(false)
    : void 0;
  var newBlocks = {};
  ...
  !(
    ...
    ? process.env.NODE_ENV !== "production"
      ? invariant(false, "position is not valid for the number of children")
      : invariant(false)
    : void 0; // add as parent's child
...
var updateSibling = function updateSibling(blockMap, prevKey, nextKey) {
  ...
  !(prevSibling != null && nextSibling != null)
    ? process.env.NODE_ENV !== "production"
      ? invariant(false, "siblings should exist in the block map")
      : invariant(false)
    : void 0;
  var newBlocks = {};
...
var replaceParentChild = function replaceParentChild(
  blockMap,
  parentKey,
  existingChildKey,
  newChildKey
...
var createNewParent = function createNewParent(blockMap, key) {
  ...
  !(block != null)
    ? process.env.NODE_ENV !== "production"
      ? invariant(false, "block must exist in block map")
      : invariant(false)
    : void 0;
  ...
  var newBlockMap = blockMap
    ...
    .concat(
      blockMap.skipUntil(function(block) {
        return block.getKey() === key;
      })
...
var updateAsSiblingsChild = function updateAsSiblingsChild(
  blockMap,
  key,
  position
) {
  ...
  !(newParentKey != null)
    ? process.env.NODE_ENV !== "production"
      ? invariant(false, "sibling is null")
      : invariant(false)
    : void 0;
  var newParent = blockMap.get(newParentKey);
  !(newParent !== null && newParent.getText() === "")
    ? process.env.NODE_ENV !== "production"
      ? invariant(false, "parent must be a valid node")
      : invariant(false)
    : void 0;
  var newBlockMap = blockMap;
    ...
    case "next":
      ...
      if (prevSibling != null) {
        ...
        newBlockMap = newBlockMap.set(
          newParentKey,
          newBlockMap.get(newParentKey).merge({
            prevSibling: null
          })
        );
...
var moveChildUp = function moveChildUp(blockMap, key) {
  ...
  !(block != null)
    ? process.env.NODE_ENV !== "production"
      ? invariant(false, "block must exist in block map")
      : invariant(false)
    : void 0; // if there is no parent, do nothing
  ...
  !(parent !== null)
    ? process.env.NODE_ENV !== "production"
      ? invariant(false, "parent must exist in block map")
      : invariant(false)
    : void 0;
  var newBlockMap = blockMap;
  ...
  !(childIndex === 0 || childIndex === parent.getChildKeys().count() - 1)
    ? process.env.NODE_ENV !== "production"
      ? invariant(false, "block is not first or last child of its parent")
      : invariant(false)
    : void 0; // If it's the first child, move as previous sibling of parent
  ...
  if (grandparentKey != null) {
    var grandparentInsertPosition = newBlockMap
      .get(grandparentKey)
      .getChildKeys()
  ...
  if (parent.getChildKeys().count() === 0) {
    ...
    if (prevSiblingKey == null && nextSiblingKey != null) {
      newBlockMap = newBlockMap.set(
        nextSiblingKey,
        newBlockMap.get(nextSiblingKey).merge({
          prevSibling: null
        })
      );
    }
    ...
    if (nextSiblingKey == null && prevSiblingKey != null) {
      newBlockMap = newBlockMap.set(
        prevSiblingKey,
        newBlockMap.get(prevSiblingKey).merge({
          nextSibling: null
        })
      );
    }
...
var mergeBlocks = function mergeBlocks(blockMap, key) {
  ...
  !(block !== null)
    ? process.env.NODE_ENV !== "production"
      ...
      : invariant(false)
    : void 0;
  !(block.getChildKeys().count() > 0)
    ? process.env.NODE_ENV !== "production"
      ? invariant(false, "block must be a non-leaf")
      : invariant(false)
    : void 0; // next block must exist & be a non-leaf

  var nextBlockKey = block.getNextSiblingKey();
  !(nextBlockKey != null)
    ? process.env.NODE_ENV !== "production"
      ? invariant(false, "block must have a next block")
      : invariant(false)
    : void 0;
  var nextBlock = blockMap.get(nextBlockKey);
  !(nextBlock != null)
    ? process.env.NODE_ENV !== "production"
      ? invariant(false, "next block must exist in block map")
      : invariant(false)
    : void 0;
  !(nextBlock.getChildKeys().count() > 0)
    ? process.env.NODE_ENV !== "production"
      ? invariant(false, "next block must be a non-leaf")
      : invariant(false)
    : void 0;
  var childKeys = block.getChildKeys().concat(nextBlock.getChildKeys());
  var newBlockMap = blockMap.set(
    key,
    block.merge({
  ...
  if (nextNextBlockKey != null) {
    newBlockMap = newBlockMap.set(
      nextNextBlockKey,
      blockMap.get(nextNextBlockKey).merge({
        prevSibling: key
      })
    );
  }
