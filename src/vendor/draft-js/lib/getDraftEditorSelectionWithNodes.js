...
function getFirstLeaf(node) {
  while (
    node.firstChild && // data-blocks has no offset
...
function getLastLeaf(node) {
  while (
    node.lastChild && // data-blocks has no offset
...
function getPointForNonTextNode(editorRoot, startNode, childOffset) {
  ...
  !(
    ...
    ? process.env.NODE_ENV !== "production"
      ? invariant(false, "Unknown node in selection range.")
      : invariant(false)
    : void 0; // If the editorRoot is the selection, step downward into the content
  // wrapper.
  ...
  if (editorRoot === node) {
    ...
    !isElement(node)
      ? process.env.NODE_ENV !== "production"
        ? invariant(false, "Invalid DraftEditorContents node.")
        : invariant(false)
      : void 0;
