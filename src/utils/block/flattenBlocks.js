...
const flattenBlocks = (blockMap, startKey, endKey) => {
  ...
  if (blocksBeforeLast) {
    ...
    prefixBlocks = prefixBlocks.set(blocksBeforeLast.getKey(), blocksBeforeLast)
  }
  ...
  if (blocksAfterFirst) {
    suffixBlocks = suffixBlocks.set(blocksAfterFirst.getKey(), blocksAfterFirst)
  }

  let blocks = prefixBlocks.toSeq().concat(newBlockMap
    .toSeq()
    ...
    .concat(suffixBlocks.toSeq())
      ...
      if (last) {
        ...
        const newCurrentBlock = currentBlock.merge({ prevSibling: lastBlockKey })
