...
const Resizable = WrappedComponent => props => {
  ...
  const onMouseMoveHandler = useCallback(
    e => {
      ...
      if (alignment === "center") {
        nextWidth.current = `${resizeModeStartNodeLayout.current.width +
          deltaX * 2}px`;
      } else {
        nextWidth.current = `${resizeModeStartNodeLayout.current.width +
          deltaX}px`;
  ...
  const onMouseEnterLeftHandler = useCallback(
    e => {
      e.preventDefault();
      if (!leftBarVisible) {
  ...
  const onMouseEnterRightHandler = useCallback(
    e => {
      e.preventDefault();
      if (!rightBarVisible) {
  ...
  const onMouseLeaveLeftHandler = useCallback(
    e => {
      e.preventDefault();
      if (leftBarVisible) {
  ...
  const onMouseLeaveRightHandler = useCallback(
    e => {
      e.preventDefault();
      if (rightBarVisible) {
